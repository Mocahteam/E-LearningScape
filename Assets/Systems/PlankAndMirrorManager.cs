...
public class PlankAndMirrorManager : FSystem {
	...
	protected override void onProcess(int familiesUpdateCount)
    {
        ...
        if (selectedPlank)
        {
            ...
            if (((f_closePlank.Count == 0 && Input.GetMouseButtonDown(0)) || (Input.GetKeyDown(KeyCode.Escape) && f_iarBackground.Count == 0) || (Input.GetButtonDown("B_Button") && f_iarBackground.Count == 0)) && !movePlank)
            {
                // ask to exit plank
                prepareClosing = true;
            ...
            if (!movePlank)
            {
                if (Input.GetKey(KeyCode.LeftArrow) || Input.GetKey(KeyCode.Q) || Input.GetAxis("Horizontal") == -1 || Input.GetAxis("Mouse X") == -1)
                    rotatePlank(1);
                if (Input.GetKey(KeyCode.RightArrow) || Input.GetKey(KeyCode.D) || Input.GetAxis("Horizontal") == 1 || Input.GetAxis("Mouse X") == 1)
                    rotatePlank(-1);
