	...
	protected override void onProcess(int familiesUpdateCount) {
		...
		if (CollectableGO.usingLamp) {
			...
			for (int i = 0; i < nbSymbols; i++) {
				...
				if (Vector3.Angle (forGO.transform.position - Camera.main.transform.position, Camera.main.transform.forward) < 30) {
					forGO.SetActive (true);
					//calculate the intersection between player direction and the wall
					float d = Vector3.Dot((new Vector3(0,0,-4.9f) - Camera.main.transform.position),Vector3.forward)/Vector3.Dot(Camera.main.transform.forward,Vector3.forward);
					forGO.transform.position = Camera.main.transform.position + Camera.main.transform.forward * d;
					forGO.GetComponentInChildren<E12_Symbol> ().gameObject.transform.position = forGO.GetComponentInChildren<E12_Symbol> ().position;
				} else {
					forGO.SetActive (false);
				}
			}
