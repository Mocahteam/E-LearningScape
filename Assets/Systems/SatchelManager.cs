...
public class SatchelManager : FSystem {
	...
	protected override void onProcess(int familiesUpdateCount)
    {
        ...
        if (selectedBag)
        {
            ...
            if (((f_closeBag.Count == 0 && Input.GetMouseButtonDown(0)) || (Input.GetButtonDown("B_Button") && f_iarBackground.Count == 0) || (Input.GetKeyDown(KeyCode.Escape) && f_iarBackground.Count == 0)) && (paperOut || bagPadlock.activeSelf))
            {
                // ask to close bag
                prepareclosing = true;
            }
