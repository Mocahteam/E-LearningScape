...
public class TakeObject : FSystem {
  ...
  public TakeObject()
  {
    ...
    foreach (GameObject go in takables) {
      go.GetComponent<Takable>().spawnPos = new Vector3(go.transform.localPosition.x, go.transform.localPosition.y, go.transform.localPosition.z);
  ...
  protected override void onPause(int currentFrame) {
    ...
    // Bring back objects that may have fallen
    foreach (GameObject go in takables) {
      if(go.transform.position.y < -10) {
        Vector3 pos = go.GetComponent<Takable>().spawnPos;
